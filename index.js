...
const _ = {
    ...
    fill(object, defaults) {
        for (const key in defaults)
            object[key] === undefined && (object[key] = _.clone(defaults[key]));
        return object;
    },
    force(target, x, y, z, relative) {
        target.setVelocity(_.local(target.getVelocity(), relative ? target.getLocation() : target.getLocation().toVector().toLocation(target.getWorld()), x, y, z));
    },
    holds(player, condition) {
    ...
    modifier(amount, operation, slot, uuid) {
